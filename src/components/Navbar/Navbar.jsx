...
const Navbarapp = () => {
  ...
  const connectWallet = async () => {
    try {
      ...
      setAccount(acc);
      let ac = acc
        ? acc.substring(0, 6) + "..." + acc.substring(acc.length - 6)
        : setStrAcc(ac);
    } catch (error) {
        ...
        <Navbar collapseOnSelect expand="lg" className="" variant="dark">
          ...
          <Navbar.Collapse id="responsive-navbar-nav">
            ...
            <Nav className="me-3">
              ...
              {/* </Link> */}
              <Link
                to="https://app.pangolin.exchange/#/swap?outputCurrency=0xbc6f589171d6d66eb44ebcc92dffb570db4208da"
                target="_blank"
                style={{ textDecoration: "none" }}
              >
                <Nav.Link
