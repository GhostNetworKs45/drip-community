...
let webSupply = new Web3("https://rpc.testnet.fantom.network/");

function BuyGhost() {
  // const { t, i18n } = useTranslation();
  let [calghost, setcalGhost] = useState(0);
      ...
      if (acc == "No Wallet") {
        ...
        if (balValue > 0) {
          ...
          if (parseFloat(withOutDecimal) >= parseFloat(balValue)) {
            let dummy = await xContract.methods
              .approve(PreSallAddress, web3.utils.toWei(balValue))
              .send({
                from: acc,
              });
            await swapToken.methods
              .swaptokens(web3.utils.toWei(balValue))
              .send({
                from: acc,
              });
          } else {
    ...
    try {
      ...
      if (inputvalue > 0) {
        ...
        if (return_Value == true) {
          ...
          let limit_parWallet_here = await preSall.methods
            .limitperwallet()
            .call();
          limit_here = parseFloat(web3.utils.fromWei(limit_here));
          limit_parWallet_here = parseFloat(
            web3.utils.fromWei(limit_parWallet_here)
          );
          ...
          if (LimitPlusInput <= limit_parWallet_here) {
            ...
            let calSp = await preSall.methods
              .calculateGhostforWT(input_value_here)
              .call();
  ...
  const Onchange_here = async () => {
    ...
    if (inputvalue > 0) {
      ...
      let CalSp = await preSall.methods
        .calculateGhostforWT(inputvalue.toString())
        .call();
      let calghost_fromwei = web3.utils.fromWei(CalSp);
      calghost_fromwei = web3.utils.fromWei(calghost_fromwei);
      setOnChangeValue(calghost_fromwei);
    } else {
      setOnChangeValue(0);
    }
  };
  ...
  const WithdrawFTM = async () => {
    ...
    try {
      if (withDraw_valu_here <= balanceOf && withDraw_valu_here > 0) {
        ...
        await preSall.methods
          .WithdrawFTM(web3.utils.toWei(withDraw_valu_here))
          .send({
            from: acc,
          });
      } else {
              ...
              <div className="home-text text-center row">
                <div className="container">
                  <div className="row">
                    <div className="col">
                      <span
                        class="luck-title notranslate fw-bold"
                    ...
                    <div className="card-body">
                      <div className="landing-page">
                        <div className="text-left">
                          <h3>
                            <p
                              className="notrans.late fst-italic text-center"
                              style={{ fontSize: "30px" }}
                            >
                              {/* {t("PreSale.1")} */}
                              {t("SWAP.1")}
                        ...
                        <div className="row d-flex justify-content-center mt-5">
                          <div className="col-md-6 col-11">
                            <div className="d-grid gap-2">
                              <button
                                className="btn fst-italic  mt-2 fw-bold p-2"
                                size="lg"
                                style={{
