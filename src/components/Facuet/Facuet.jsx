...
const Facuet = ({ oneTokenPrice }) => {
  ...
  const getData = async () => {
    ...
    if (acc == "No Wallet") {
      ...
      try {
        ...
        let myclaimsAvailable = await contractOf.methods
          .claimsAvailable(acc)
          .call();
        ...
        if (parseFloat(enteredAirVal) > 0) {
          if (enteredAddrs.length > 10) {
            if (parseFloat(userDripBalance) > parseFloat(enteredAirVal)) {
              ...
              let usersinf = await contractOf.methods
                .users(enteredAddrs)
                .call();
              ...
              let ownwerAddrss = await contractOf.methods
                .dripVaultAddress()
                .call();
              ...
              if (
                ...
                await tokenContractOf.methods
                  .approve(faucetContractAddress, enteredAirVal)
                  .send({
                    from: acc,
                  });
                ...
                await contractOf.methods
                  .airdrop(enteredAddrs, enteredAirVal)
                  .send({
                    from: acc,
                  });
                toast.success("Transaction confirmed");
  ...
  const custody = async () => {
    ...
    if (acc == "No Wallet") {
      ...
      try {
        ...
        let contractOfBuddy = new web3.eth.Contract(
          buddySystemAbi,
          buddySystemAddress
        );
  ...
  const goPlayerinfo = async () => {
    ...
    try {
      ...
      let userInfoTotal = await contractOf.methods
        .userInfoTotals(enteredAddress)
        .call();
      let playeruserInfo = await contractOf.methods
        .userInfo(enteredAddress)
        .call();
      let totalUsers = await contractOf.methods.total_users().call();
      ...
      toast.error(
        "Can't Fetch User's Information at the moment please try again later."
      );
      console.log("error", e);
    }
  };
      ...
      if (acc == "No Wallet") {
        ...
        if (enteredVal >= 1) {
          if (parseFloat(userDripBalance) >= parseFloat(enteredVal)) {
            ...
            let isWhiteList = await tokenContractOf.methods
              .whitelist(acc)
              .call();
            let isExcluded = await tokenContractOf.methods
              .isExcluded(acc)
              .call();
            if (referral.length > 15) {
              ...
              await tokenContractOf.methods
                .approve(faucetContractAddress, web3.utils.toWei(enteredVal))
                .send({
                  from: acc,
                });
              toast.success("Transaction confirmed");
          ...
          if (parseFloat(userDripBalance) > parseFloat(enteredVal)) {
            ...
            if (referral != "0x0000000000000000000000000000000000000000") {
              ...
              if (allowance >= parseFloat(web3.utils.toWei(enteredVal))) {
                ...
                toast.error(
                  "Entered value is greater than your approval amount "
                );
              }
            } else {
              toast.error("You have no Buddy.");
      ...
      if (acc == "No Wallet") {
        ...
        if (airDropPlayerAddress.current.value > 0) {
          if (budgetRef.current.value > 0) {
            // userDripBalance
            if (
              parseFloat(userDripBalance) >= parseFloat(budgetRef.current.value)
            ) {
              let data = {
              ...
              if (referralData.data.length) {
                ...
                if (filterReferral.length) {
                  if (checkCompaign == 0) {
                    ...
                    let sAdd = [];
                    let amount =
                      budgetRef.current.value / filterReferral.length;
                    ...
                    let checkStatus = filterReferral.map(async (item) => {
                      return await faucetContract.methods
                        .isNetPositive(item.value.entered_address)
                        .call();
                    });
                    ...
                    filterReferral
                      .slice(0, filterReferral.length)
                      .forEach((item) => {
                        let deposit = window.web3.utils.fromWei(
                          item.value.deposits
                        );
                    ...
                    setEstimatePerPerson(parseFloat(amount).toFixed(2));
                    setSendEstimateAmount(amount);
                    let checkStatus = filterReferral
                      .slice(0, 1)
                      .map(async (item) => {
                        return await faucetContract.methods
                          .isNetPositive(item.value.entered_address)
                    ...
                    if (filterReferral.length < 5) {
                      toast.error(
                        "Your Referrals are less than the selected compaign"
                      );
                    } else {
                      let dataAdd = [];
                      let sAdd = [];
            ...
            if (amount <= parseFloat(all)) {
              ...
              if (budgetVal > 0) {
                if (budgetVal <= sendAddress.length) {
                  ...
                  let facutContract = new web3.eth.Contract(
                    faucetContractAbi,
                    faucetContractAddress
                  );
                  ...
                  await facutContract.methods
                    .MultiSendairdrop(newArr, tosendEstimateAmount)
                    .send({ from: acc });
                  toast.success("Transaction confirmed");
                } else {
              ...
              toast.error(
                "The entered amount is greater than your approval amount"
              );
            }
          } else {
            toast.error("No recipient found");
          ...
          <div className="landing-page">
            <div className="row mb-4 mt-2">
                    ...
                    <div className="row">
                      <div className="col">
                        <span className="luck-title fw-bold notranslate">
                          <b style={{ fontFamily: "Jost" }}>{t("THETAP.1")}</b>
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="container col-xl-6 col-lg-6 col-md-6 mb-4 pt-4">
                <div id="topStatsContainer" className="row">
                  ...
                  <div className="container col-6 col-xl-4 col-lg-4 col-md-4 text-center mt-md-4">
                    <div className="price-top-part">
                      ...
                      <h5
                        className="mb-0 font-weight-semibold color-theme-1 mb-2 mt-1 fst-italic"
                        style={{ color: "#7c625a" }}
                      >
                        {t("Rewarded.1")}{" "}
                      </h5>
                ...
                <p
                  className="col-12 white mb-3 text-justify fst-italic text-white mt-md-3"
                  style={{ fontSize: "20px" }}
                >
                  {" "}
                  {t(
                ...
                <p
                  className="col-12 white mb-3 text-justify fst-italic text-white"
                  style={{ fontSize: "20px" }}
                >
                  {" "}
                  {t(
                        ...
                        >
                          {isChange == "Viewer" ? (
                            <div className="row" id="Viewerpart">
                              ...
                              <div
                                className="col-md-5 col-12 ml-md-auto mt-md-1 mt-3"
                                style={{
                                  backgroundColor: "#86ad74",
                                  overflowY: "scroll",
                                  height: "170px",
                            ...
                            <div>
                                ...
                                <div className="col-md-3 mt-3">
                                  ...
                                  <form className="">
                                    <div class="select-wrapper ">
                                      ...
                                      >
                                        <option value="0">{t("None.1")}</option>
                              ...
                              <div className="row">
                                ...
                                <div className="col-md-7">
                                  ...
                                  <div
                                    className="row my-custom-scrollbar"
                                    responsive="sm"
                                  >
